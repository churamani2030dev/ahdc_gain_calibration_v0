...
//  - MPV extraction from SUB using Landau fits (per wire)
//  - Per-wire gain from MPV_ref / MPV_wire (FIT only)
//  - Proper error propagation for gains and MPV_corr
//    *MPV errors estimated from histogram width & statistics, not from fit par errors*
//  - Green Landau overlays for global & per-wire fits
//  - Deuteron-only and proton-only 2D SUB pieces (both visible)
...
static double deg0to360(double a){
  double x=a%360.0d; return (x<0.0d)? x+360.0d : x
}
...
static double dphi0to360(double pe,double pk){
  double x=((pe - pk)%360.0d); return (x<0.0d)? x+360.0d : x
}
...
@Field H2F PTADC_BEFORE = new H2F("ptadc_before",
  "ADC vs p_{T}(e') — BEFORE cuts; p_{T} (GeV); ADC",
  PT_NBINS_2D, PT_MIN_2D, PT_MAX_2D,
  ADC_NBINS_2D, ADC_MIN_2D, ADC_MAX_2D)

@Field H2F PTADC_SIG = new H2F("ptadc_sig",
  "ADC vs p_{T}(e') — SIG (all cuts); p_{T} (GeV); ADC",
  PT_NBINS_2D, PT_MIN_2D, PT_MAX_2D,
  ADC_NBINS_2D, ADC_MIN_2D, ADC_MAX_2D)

@Field H2F PTADC_BKG = new H2F("ptadc_bkg",
  "ADC vs p_{T}(e') — BKG; p_{T} (GeV); ADC",
  PT_NBINS_2D, PT_MIN_2D, PT_MAX_2D,
  ADC_NBINS_2D, ADC_MIN_2D, ADC_MAX_2D)
...
// -------- NEW: deuteron-only and proton-only pieces of SUB ----------
@Field double[][] PTADC_SUB_D_ARRAY      = new double[PT_NBINS_2D][ADC_NBINS_2D]
@Field double[][] PTADC_SUB_D_GAIN_ARRAY = new double[PT_NBINS_2D][ADC_NBINS_2D]
@Field H2F PTADC_SUB_D = new H2F("ptadc_sub_D",
  "ADC (SUB_D) vs p_{T}(e') — deut only; p_{T} (GeV); ADC",
  PT_NBINS_2D, PT_MIN_2D, PT_MAX_2D,
  ADC_NBINS_2D, ADC_MIN_2D, ADC_MAX_2D)
@Field H2F PTADC_SUB_D_GAIN = new H2F("ptadc_sub_D_gain",
  "ADC_{corr} (SUB_D) vs p_{T}(e') — deut only; p_{T} (GeV); ADC_{corr}",
  PT_NBINS_2D, PT_MIN_2D, PT_MAX_2D,
  ADC_NBINS_2D, ADC_MIN_2D, ADC_MAX_2D)

@Field double[][] PTADC_SUB_P_ARRAY      = new double[PT_NBINS_2D][ADC_NBINS_2D]
@Field double[][] PTADC_SUB_P_GAIN_ARRAY = new double[PT_NBINS_2D][ADC_NBINS_2D]
@Field H2F PTADC_SUB_P = new H2F("ptadc_sub_P",
  "ADC (SUB_P) vs p_{T}(e') — proton piece; p_{T} (GeV); ADC",
  PT_NBINS_2D, PT_MIN_2D, PT_MAX_2D,
  ADC_NBINS_2D, ADC_MIN_2D, ADC_MAX_2D)
@Field H2F PTADC_SUB_P_GAIN = new H2F("ptadc_sub_P_gain",
  "ADC_{corr} (SUB_P) vs p_{T}(e') — proton piece; p_{T} (GeV); ADC_{corr}",
  PT_NBINS_2D, PT_MIN_2D, PT_MAX_2D,
  ADC_NBINS_2D, ADC_MIN_2D, ADC_MAX_2D)
...
double estimateMPVError(H1F h, double mpv, double xmin, double xmax){
  ...
  return sigmaWidth / Math.sqrt(sumW)   // ~ width / sqrt(N)
}
...
FitResultGlobal fitGlobalLandau(H1F h, double xmin, double xmax){
  ...
  // MPV error from histogram width & N (NOT fit par error)
...
FitResult1D fitLandau1D(H1F h, double xmin, double xmax, String tag, int nSampleBins){
  ...
  // Use error from histogram formula instead of fit parameter errors
  out.ampErr = 0.0
  out.sigErr = 0.0
  out.mpvErr = estimateMPVError(h, out.mpv, xmin, xmax)
...
opt.addCommand("process","Per-wire ADC with BKG subtraction, Landau MPV (FIT only), and gain extraction")
...
if(opt.getCommand()!="process"){
  System.err.println("Usage: run-groovy gain_MPV_fit_only.groovy process [opts] files.hipo ...")
  System.exit(1)
}
...
try{ v=po.getOption("-fdonly")?.getValue(); if(v!=null) FD_ONLY=Boolean.parseDouble(v.trim()) }catch(Exception ignore){}
...
for(String fn : files){
  ...
  try{ R.open(fn) }catch(Exception ex){
    System.err.println("Open fail "+fn+" : "+ex); continue
  }
...
Map<WireKey,Double> mpvCorrMap    = new LinkedHashMap<>()
Map<WireKey,Double> mpvCorrErrMap = new LinkedHashMap<>()

Map<WireKey,Double> gainMap       = new LinkedHashMap<WireKey,Double>()  // used for correction

// pre-gain fits from SUB, and gains (FIT ONLY)
for(Map.Entry<WireKey,PairH> e : histMap.entrySet()){
  WireKey wk = e.getKey()
  PairH  ph = e.getValue()
  ...
  double g_fit = 1.0
  double g_fit_err = 0.0
  double mpv_corr = Double.NaN
  double mpv_corr_err = 0.0
  ...
  if(fr.ok && MPV_REF>0 && fr.mpv>0){
    ...
    // gain-corrected MPV from FIT: mpv_corr = mpv_pre * g_fit
    mpv_corr = fr.mpv * g_fit

    // error propagation: (σ_corr / mpv_corr)^2 = (σ_pre/mpv_pre)^2 + (σ_g/g)^2
    double rel_pre = (fr.mpvErr>0 && fr.mpv>0)? (fr.mpvErr/fr.mpv) : 0.0
    double rel_g   = (g_fit_err>0 && g_fit>0)? (g_fit_err/g_fit)   : 0.0
    double rel_tot = Math.sqrt(rel_pre*rel_pre + rel_g*rel_g)
    mpv_corr_err   = mpv_corr * rel_tot
  }

  gainFitMap.put(wk, g_fit)
  gainFitErrMap.put(wk, g_fit_err)
  gainMap.put(wk, g_fit)
  mpvCorrMap.put(wk, mpv_corr)
  mpvCorrErrMap.put(wk, mpv_corr_err)

  if(fr.ok){
    System.out.printf(Locale.ROOT,
      "PRE MPV (FIT) %s : %.1f ± %.2f ADC (chi2/ndf=%.1f/%d)  | gain_fit=%.3f ± %.3f  | MPV_corr=%.1f ± %.2f%n",
      wk.toString(), fr.mpv, fr.mpvErr, fr.chi2, fr.ndf, g_fit, g_fit_err, mpv_corr, mpv_corr_err)
  } else {
    System.out.printf(Locale.ROOT,
      "PRE MPV (FIT) %s : not fitted (gain=1 used)%n", wk.toString())
  }
}
...
if(!gainMap.isEmpty()){
  ...
  for(String fn : files){
    HipoDataSource R=new HipoDataSource()
    try{ R.open(fn) }catch(Exception ex){
      System.err.println("2nd pass open fail "+fn+" : "+ex); continue
    }
    while(R.hasEvent()){
      DataEvent ev
      try{ ev=R.getNextEvent() }catch(Exception ex){ break }
...
if(!gainMap.isEmpty() && !alphaMap.isEmpty()){
  ...
  for(String fn : files){
    HipoDataSource R=new HipoDataSource()
    try{ R.open(fn) }catch(Exception ex){
      System.err.println("3rd pass open fail "+fn+" : "+ex); continue
    }
    while(R.hasEvent()){
      DataEvent ev
      try{ ev=R.getNextEvent() }catch(Exception ex){ break }
      ...
      if(ev.hasBank("AHDC::adc")){
        ...
        for(int i=0;i<a.rows();i++){
          ...
          // weights: total SUB, deut-only, proton-only
          // SUB total still uses +1 (SIG) and -α (BKG).
          // For diagnostic "proton piece" plots we store +α·BKG so they are visible.
          double weightTotal = ELASTIC ? 1.0d : -aWire   // SUB = SIG − α·BKG
          double weightD     = ELASTIC ? 1.0d : 0.0d     // deuteron (SIG) only
          double weightP     = ELASTIC ? 0.0d : aWire    // proton/BKG magnitude (positive)
          ...
          if(x>=PT_MIN_2D && x<PT_MAX_2D && y>=ADC_MIN_2D && y<ADC_MAX_2D){
            ...
            if(ix>=0 && ix<PT_NBINS_2D && iy>=0 && iy<ADC_NBINS_2D){
              PTADC_SUB_ARRAY[ix][iy]      += weightTotal
              PTADC_SUB_D_ARRAY[ix][iy]    += weightD
              PTADC_SUB_P_ARRAY[ix][iy]    += weightP
            }
          }
          ...
          if(x>=PT_MIN_2D && x<PT_MAX_2D && yCorr>=ADC_MIN_2D && yCorr<ADC_MAX_2D){
            ...
            if(ix2>=0 && ix2<PT_NBINS_2D && iy2>=0 && iy2<ADC_NBINS_2D){
              PTADC_SUB_GAIN_ARRAY[ix2][iy2]      += weightTotal
              PTADC_SUB_D_GAIN_ARRAY[ix2][iy2]    += weightD
              PTADC_SUB_P_GAIN_ARRAY[ix2][iy2]    += weightP
            }
          }
        }
      }
  ...
  for(int ix=0; ix<PT_NBINS_2D; ix++){
    for(int iy=0; iy<ADC_NBINS_2D; iy++){
      double vTot  = PTADC_SUB_ARRAY[ix][iy]
      double vTotG = PTADC_SUB_GAIN_ARRAY[ix][iy]
      double vD    = PTADC_SUB_D_ARRAY[ix][iy]
      double vDG   = PTADC_SUB_D_GAIN_ARRAY[ix][iy]
      double vP    = PTADC_SUB_P_ARRAY[ix][iy]
      double vPG   = PTADC_SUB_P_GAIN_ARRAY[ix][iy]

      PTADC_SUB.setBinContent(ix, iy, vTot)
      PTADC_SUB_GAIN.setBinContent(ix, iy, vTotG)

      PTADC_SUB_D.setBinContent(ix, iy, vD)
      PTADC_SUB_D_GAIN.setBinContent(ix, iy, vDG)

      PTADC_SUB_P.setBinContent(ix, iy, vP)
      PTADC_SUB_P_GAIN.setBinContent(ix, iy, vPG)
    }
  }
}

// ---------------------- Post-gain MPV fits (for QA only) -----------
...
tabs.add("pT vs ADC (SUB D/P)")   // NEW tab
tabs.add("SUM 1D (SIG/BKG/SUB)")
tabs.add("Wire MPV & Gain")
tabs.add("MPV PRE vs MPV_corr")
...
// per-wire SUB with pre-fit overlays
...
// per-wire SUB (orig vs gain-corr) overlay with post-fit
...
// NEW: show deuteron/proton decomposition of SUB before/after gain
def c2dDP = canv.getCanvas("pT vs ADC (SUB D/P)")
c2dDP.divide(2,2)
// top row: deuteron piece
c2dDP.cd(0); c2dDP.draw(PTADC_SUB_D)       // deut before gain
c2dDP.cd(1); c2dDP.draw(PTADC_SUB_D_GAIN)  // deut after gain
// bottom row: proton/background piece (positive α·BKG magnitude)
c2dDP.cd(2); c2dDP.draw(PTADC_SUB_P)       // proton before gain
c2dDP.cd(3); c2dDP.draw(PTADC_SUB_P_GAIN)  // proton after gain
...
// --------- Graphs: FIT MPV & gain with error bars -----------------
GraphErrors gMPVpre_fit  = new GraphErrors("MPV_pre_fit_vs_wire")
GraphErrors gMPVcorr_fit = new GraphErrors("MPV_corr_fit_vs_wire")
...
for(WireKey wk : allKeys){
  ...
  Double mpCorr      = mpvCorrMap.get(wk)
  Double mpCorrErr   = mpvCorrErrMap.get(wk)

  if(frPre!=null && frPre.ok){
    double ey = (frPre.mpvErr>0 ? frPre.mpvErr : 0.0)
    gMPVpre_fit.addPoint(x, frPre.mpv, 0.0, ey)
  }

  if(mpCorr!=null && !mpCorr.isNaN()){
    double eyc = (mpCorrErr!=null && mpCorrErr>0.0) ? mpCorrErr : 0.0
    gMPVcorr_fit.addPoint(x, mpCorr, 0.0, eyc)
  }

  if(gft!=null && gft>0.0){
    double eyg = (gftErr!=null && gftErr>0.0) ? gftErr : 0.0
    gGAIN_fit.addPoint(x, gft, 0.0, eyg)
  }
...
// Titles
gMPVpre_fit.setTitle("MPV(pre, FIT) vs wire;wire index;MPV_{pre}^{fit} (ADC)")
gMPVcorr_fit.setTitle("MPV(corr, FIT) vs wire;wire index;MPV_{corr}^{fit} (ADC)")
gGAIN_fit.setTitle("Gain vs wire (FIT method);wire index;gain")

// Styling
[gMPVpre_fit, gMPVcorr_fit, gGAIN_fit].each{ g->
  g.setMarkerStyle(2)
  g.setMarkerSize(4)
}

// Color coding
gMPVpre_fit.setMarkerColor(1);  gMPVpre_fit.setLineColor(1)  // black
gMPVcorr_fit.setMarkerColor(2); gMPVcorr_fit.setLineColor(2) // red
gGAIN_fit.setMarkerColor(3);    gGAIN_fit.setLineColor(3)    // green

// Canvases for graphs
def cwg = canv.getCanvas("Wire MPV & Gain")
cwg.divide(2,2)
cwg.cd(0); cwg.draw(gMPVpre_fit)
cwg.cd(1); cwg.draw(gMPVcorr_fit)
cwg.cd(2); cwg.draw(gGAIN_fit)
...
// Overlay PRE vs MPV_corr (FIT-only) on separate tab
def cmpv = canv.getCanvas("MPV PRE vs MPV_corr")
cmpv.divide(1,1)
cmpv.cd(0)
cmpv.draw(gMPVpre_fit)
cmpv.draw(gMPVcorr_fit, "same")
...
System.out.printf(Locale.ROOT, "Proton veto (SIG)        : %s [%.0f,%.0f] ADC%n",
                  PROTON_VETO_ON ? "ON" : "OFF", PROT_VETO_MIN, PROT_VETO_MAX)
...
JFrame f = new JFrame(String.format(
  ...
  PT_SLICE_MIN, PT_SLICE_MAX, CTRL_LO, CTRL_HI,
  PROTON_VETO_ON ? "ON" : "OFF", pages, DRAW_COLS, DRAW_ROWS))
