// gain_alert_MPV.groovy
//
// Per-wire ADC with background subtraction, Landau fits, gain extraction,
// re-plotted SUB histograms after gain calibration, pT(e') vs ADC (SUB)
// before/after gain, and MPV vs wire both BEFORE and AFTER gain.
//
//  - First pass:
//      * build per-wire SIG/BKG histograms
//      * determine per-wire α from control ADC window
//      * build SUB = SIG − α·BKG
//      * fit SUB with Landau to get MPV per wire
//      * fit global SUM_SUB to get reference MPV_ref
//  - Compute gain per wire:  gain(wire) = MPV_ref / MPV_wire
//  - Build per-wire gain-corrected SUB histos from existing SUB
//  - Second pass:
//      * re-run over events and fill pT vs ADC (gain-corrected) +
//        global SIG 1D after gain, with all cuts applied (SIG sample).
//  - Third pass:
//      * using per-wire α and gain(wire), build pT vs ADC (SUB) 2D
//        before and after gain (weighted: +1 for SIG, −α for BKG).
//  - Post-gain MPV:
//      * fit per-wire gain-corrected SUB histos with same Landau
//      * plot MPV_post vs wire index in a separate GUI.
//
//  Example:
//    run-groovy gain_alert_MPV.groovy process \
...
// robust χ²/ndf evaluation over bins inside [xmin,xmax]
FitResult fitDeuteronPeak(H1F h, double xminFit, double xmaxFit){
  FitResult out = new FitResult()
  if(h.integral() < 50.0){ return out }   // too few entries

  int binMax = h.getMaximumBin()
  double mpv0 = h.getAxis().getBinCenter(binMax)
  double amp0 = h.getBinContent(binMax)
  double sig0 = Math.max(50.0, (xmaxFit-xminFit)/10.0)

  F1D f = new F1D("f","[A]*landau(x,[MPV],[SIG])", xminFit, xmaxFit)
  f.setParameter(0, amp0)
  f.setParameter(1, mpv0)
  f.setParameter(2, sig0)

  DataFitter.fit(f, h, "Q")  // silent fit

  out.amp   = f.getParameter(0)
  out.mpv   = f.getParameter(1)
  out.sigma = f.getParameter(2)
  out.ok    = !Double.isNaN(out.mpv) && out.mpv>0
  out.params = new double[]{out.amp, out.mpv, out.sigma}
  ...
  try{
    ...
    for(int b=0;b<nb;b++){
      ...
      if(x<xminFit || x>xmaxFit) continue
      double y = h.getBinContent(b)
      double ye = Math.max(1.0, Math.sqrt(Math.max(0.0,y))) // simple Poisson σ
      double yfit = f.evaluate(x)
      double d = (y - yfit)/ye
      chi2 += d*d
      nUsed++
    }
    out.ndf  = (nUsed>3 ? nUsed-3 : 0)
    out.chi2 = (out.ndf>0 ? chi2 : 0.0)
...
opt.addCommand("process","Per-wire ADC with BKG subtraction, Landau fits, and gain extraction")
...
if(opt.getCommand()!="process"){
  System.err.println("Usage: run-groovy gain_alert_MPV.groovy process [opts] files.hipo ...")
  System.exit(1)
}
...
// --------------------------- Landau fits (pre-gain) -----------------
@Field Map<WireKey,FitResult> mpvMap  = new LinkedHashMap<WireKey,FitResult>()
@Field Map<WireKey,Double>    gainMap = new LinkedHashMap<WireKey,Double>()

// choose a common fit range (roughly where deuteron peak lives)
@Field double FIT_LO = 450.0d
@Field double FIT_HI = 2000.0d

// per-wire fits + visible fit curves (SUB pre-gain)
for(Map.Entry<WireKey,PairH> e : histMap.entrySet()){
  WireKey wk = e.getKey()
  PairH  ph  = e.getValue()
  ...
  if(fr!=null && fr.ok){
    System.out.printf(Locale.ROOT,
      "MPV %s : %.1f ADC   (chi2/ndf = %.1f/%d)%n",
      wk.toString(), fr.mpv, fr.chi2, fr.ndf)
    ...
    System.out.printf(Locale.ROOT,
      "MPV %s : not fitted (too few entries)%n", wk.toString())
  }
}
...
if(frSum!=null && frSum.ok){
  MPV_REF = frSum.mpv
  System.out.printf(Locale.ROOT,
    "%nGlobal SUM_SUB MPV = %.1f ADC  (chi2/ndf = %.1f/%d)%n",
    frSum.mpv, frSum.chi2, frSum.ndf)
  ...
  System.out.println("\nGlobal SUM_SUB MPV: not fitted (too few entries)")
}

// --------------------------- Gain per wire --------------------------
if(!Double.isNaN(MPV_REF)){
  for(Map.Entry<WireKey,FitResult> e : mpvMap.entrySet()){
    WireKey wk = e.getKey()
...
// ------------------ Post-gain Landau fits (per-wire) ----------------
for(Map.Entry<WireKey,H1F> e : subGainMap.entrySet()){
  WireKey wk = e.getKey()
  H1F    hCorr = e.getValue()
  FitResult fr = fitDeuteronPeak(hCorr, FIT_LO, FIT_HI)
  mpvPostMap.put(wk, fr)

  if(fr!=null && fr.ok){
    System.out.printf(Locale.ROOT,
      "POST-GAIN MPV %s : %.1f ADC   (chi2/ndf = %.1f/%d)%n",
      wk.toString(), fr.mpv, fr.chi2, fr.ndf)
  }else{
    System.out.printf(Locale.ROOT,
      "POST-GAIN MPV %s : not fitted (too few entries)%n", wk.toString())
  }
}
...
// MPV & gain vs wire (pre-gain MPV, gain; post-gain MPV)
GraphErrors gMPV      = new GraphErrors("MPV_vs_wire")
GraphErrors gGAIN     = new GraphErrors("GAIN_vs_wire")
GraphErrors gMPV_post = new GraphErrors("MPV_post_vs_wire")
int idx=0
for(WireKey wk : allKeys){
  FitResult fr   = mpvMap.get(wk)
  FitResult frPg = mpvPostMap.get(wk)
  double mpv = (fr!=null && fr.ok)? fr.mpv : Double.NaN
  double mpvPost = (frPg!=null && frPg.ok)? frPg.mpv : Double.NaN
  double gain = gainMap.containsKey(wk)? gainMap.get(wk) : 1.0d
  double x = (double)idx
  if(!Double.isNaN(mpv)){
    gMPV.addPoint(x, mpv, 0, 0)
  }
  if(!Double.isNaN(mpvPost)){
    gMPV_post.addPoint(x, mpvPost, 0, 0)
  }
  gGAIN.addPoint(x, gain, 0, 0)
  idx++
}
gMPV.setTitle("MPV vs wire index;wire index;MPV (ADC)")
gGAIN.setTitle("Gain vs wire index;wire index;gain (MPV_ref / MPV_wire)")
gMPV_post.setTitle("Post-gain MPV vs wire index;wire index;MPV_{post} (ADC)")

def cwg = canv.getCanvas("Wire MPV & Gain")
cwg.divide(1,2)
cwg.cd(0); cwg.draw(gMPV)
cwg.cd(1); cwg.draw(gGAIN)

// y-axis limits: MPV ≤ 1500, gain ∈ [0,3]
try{
  def pad0 = cwg.getPad(0)
  pad0.getAxisFrame().getAxisY().setRange(0.0, 1500.0)
} catch(Exception ignore){}
try{
  def pad1 = cwg.getPad(1)
  pad1.getAxisFrame().getAxisY().setRange(0.0, 3.0)
} catch(Exception ignore){}
...
// -------- Separate GUI: MPV_post vs wire ----------------------------
EmbeddedCanvasTabbed canvPost = new EmbeddedCanvasTabbed("Post-gain MPV vs wire")
def cp = canvPost.getCanvas("Post-gain MPV vs wire")
cp.divide(1,1)
cp.cd(0)
cp.draw(gMPV_post)
try{
  cp.getPad(0).getAxisFrame().getAxisY().setRange(0.0, 1500.0)
}catch(Exception ignore){}

JFrame fp = new JFrame("Post-gain MPV vs wire index")
fp.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE)
fp.setSize(900,600); fp.add(canvPost); fp.setLocationRelativeTo(null); fp.setVisible(true)
...
if(!Double.isNaN(MPV_REF)){
  System.out.printf(Locale.ROOT, "Global MPV_ref (SUM_SUB) : %.1f ADC%n", MPV_REF)
}
System.out.println("-----------------------------------------")
