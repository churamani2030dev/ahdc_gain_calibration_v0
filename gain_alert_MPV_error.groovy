// Fast & robust version:
//
// Per-wire ADC with background subtraction, gain extraction,
// pT(e') vs ADC (SUB) before/after gain,
// MPV vs wire (pre & post gain) with error bars,
// gain vs wire with error bars.
//
// Main change from previous versions:
//  - NO Minuit/DataFitter for per-wire fits.
//  - Per-wire MPV is extracted from the SUB histograms using
//    a simple peak finder + parabolic interpolation around the max,
//    with FWHM-based width and an approximate MPV error.
//  - Only the global SUM_SUB histogram is fitted with Landau
//    (once) to get MPV_ref and its error.
//
// Usage example:
//   run-groovy gain_alert_MPV_fast.groovy process \
...
PeakResult extractPeak(H1F h, double xminFit, double xmaxFit){
  ...
  // parabolic interpolation around max bin (assuming equal bin width)
  ...
  if(Math.abs(denom) > 1e-6){
    ...
    if(Math.abs(delta) < 1.0){ // don't go crazy
      xPeak = x2 + delta*bw
    }
  }
  ...
  // approximate MPV error ~ sigma / sqrt(Nwindow)
...
FitResultGlobal fitGlobalLandau(H1F h, double xmin, double xmax){
  ...
  try{
    DataFitter.fit(f, h, "Q")  // only one global fit
  }catch(Exception ex){
    return out
  }
...
opt.addCommand("process","Per-wire ADC with BKG subtraction, peak-based MPV, and gain extraction")
...
if(opt.getCommand()!="process"){
  System.err.println("Usage: run-groovy gain_alert_MPV_fast.groovy process [opts] files.hipo ...")
  System.exit(1)
}
...
// --------------------------- MPV (pre), gain per wire ---------------
@Field Map<WireKey,PeakResult> mpvPreMap  = new LinkedHashMap<WireKey,PeakResult>()
@Field Map<WireKey,PeakResult> mpvPostMap = new LinkedHashMap<WireKey,PeakResult>()
@Field Map<WireKey,Double>     gainMap    = new LinkedHashMap<WireKey,Double>()

// pre-gain peak from SUB
for(Map.Entry<WireKey,PairH> e : histMap.entrySet()){
  WireKey wk = e.getKey()
  PairH  ph  = e.getValue()
  ...
  if(acceptPeak(pr)){
    System.out.printf(Locale.ROOT,
      "MPV %s : %.1f ± %.2f ADC   (sigma ≈ %.1f, Nwin=%.1f)%n",
      wk.toString(), pr.mpv, pr.mpvErr, pr.sigma, pr.integral)
  }else{
    System.out.printf(Locale.ROOT,
      "MPV %s : not used (too few entries or bad peak)%n", wk.toString())
  }
}

// gain map from MPV_REF / MPV_pre
gainMap.clear()
if(!Double.isNaN(MPV_REF) && MPV_REF>0){
  for(Map.Entry<WireKey,PeakResult> e : mpvPreMap.entrySet()){
    WireKey wk = e.getKey()
    PeakResult pr = e.getValue()
    double g = 1.0
    if(acceptPeak(pr) && pr.mpv>0){
      g = MPV_REF / pr.mpv
    }
    gainMap.put(wk, g)
  }
...
if(!gainMap.isEmpty() && !alphaMap.isEmpty()){
  ...
  for(String fn : files){
    ...
    while(R.hasEvent()){
      ...
      // banana for same SIG/BKG split as pass1
      long sumTRK = 0L
      if(ev.hasBank("AHDC::adc")){
        DataBank a0 = ev.getBank("AHDC::adc")
        for(int i=0;i<a0.rows();i++){
...
// ---------------------- Post-gain peak on SUB_gain ------------------
for(Map.Entry<WireKey,H1F> e : subGainMap.entrySet()){
  WireKey wk = e.getKey()
  H1F hCorr  = e.getValue()
  PeakResult prp = extractPeak(hCorr, FIT_LO, FIT_HI)
  mpvPostMap.put(wk, prp)

  if(acceptPeak(prp)){
    System.out.printf(Locale.ROOT,
      "POST-GAIN MPV %s : %.1f ± %.2f ADC   (sigma ≈ %.1f, Nwin=%.1f)%n",
      wk.toString(), prp.mpv, prp.mpvErr, prp.sigma, prp.integral)
  }else{
    System.out.printf(Locale.ROOT,
      "POST-GAIN MPV %s : not used (too few entries or bad peak)%n", wk.toString())
  }
}
...
// MPV(pre), MPV(post) & gain vs wire (with errors)
...
double fracRefErr = 0.0
if(MPV_REF>0 && MPV_REF_ERR>0){
  fracRefErr = MPV_REF_ERR / MPV_REF
}

for(WireKey wk : allKeys){
  PeakResult prPre  = mpvPreMap.get(wk)
  PeakResult prPost = mpvPostMap.get(wk)
  double gain = gainMap.containsKey(wk)? gainMap.get(wk) : 1.0d
  double x = (double)idxWire

  if(acceptPeak(prPre)){
    gMPVpre.addPoint(x, prPre.mpv, 0.0, prPre.mpvErr)
  }
  if(acceptPeak(prPost)){
    gMPVpost.addPoint(x, prPost.mpv, 0.0, prPost.mpvErr)
  }

  if(acceptPeak(prPre) && MPV_REF>0){
    double g = gain
    double fracWireErr = 0.0
    if(prPre.mpv>0 && prPre.mpvErr>0){
      fracWireErr = prPre.mpvErr / prPre.mpv
    }
    double gErr = 0.0
    if(fracRefErr>0 || fracWireErr>0){
      double rel2 = fracRefErr*fracRefErr + fracWireErr*fracWireErr
      gErr = g * Math.sqrt(rel2)
    }
    gGAIN.addPoint(x, g, 0.0, gErr)
  }

  idxWire++
}

gMPVpre.setTitle("MPV (PRE gain) vs wire index;wire index;MPV_{pre} (ADC)")
gMPVpost.setTitle("MPV (POST gain) vs wire index;wire index;MPV_{post} (ADC)")
gGAIN.setTitle("Gain vs wire index;wire index;gain (MPV_ref / MPV_pre)")

def cwg = canv.getCanvas("Wire MPV & Gain")
cwg.divide(1,2)
cwg.cd(0); cwg.draw(gMPVpre)
cwg.cd(1); cwg.draw(gGAIN)

// y-axis limits for main canvas: MPV and gain
try{
...
// Overlay MPV(pre) & MPV(post) in separate tab
def cmpv = canv.getCanvas("MPV PRE vs POST")
cmpv.divide(1,1)
cmpv.cd(0)
gMPVpre.setMarkerColor(1); gMPVpre.setLineColor(1)
gMPVpost.setMarkerColor(2); gMPVpost.setLineColor(2)
cmpv.draw(gMPVpre)
cmpv.draw(gMPVpost, "same")
