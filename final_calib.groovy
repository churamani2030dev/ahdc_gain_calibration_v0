...
FitResultGlobal fitGlobalLandau(H1F h, double xmin, double xmax){
  ...
  try{ out.mpvErr = f.getParError(1) }catch(Exception ignore){ out.mpvErr = 0.0 }
...
for(Map.Entry<WireKey,PairH> e : histMap.entrySet()){
  ...
  if(fr.ok && MPV_REF>0 && fr.mpv>0){
    ...
    // if fit failed, keep gain=1, mpv_corr=NaN
  }
...
for(WireKey wk : allKeys){
  ...
  if(frPre!=null && frPre.ok){
    gMPVpre_fit.addPoint(x, frPre.mpv, 0.0, (frPre.mpvErr>0? frPre.mpvErr:0.0))
  }
  if(mpCorr!=null && !mpCorr.isNaN()){
    gMPVcorr_fit.addPoint(x, mpCorr, 0.0, (mpCorrErr!=null? mpCorrErr:0.0))
  }
  if(gft!=null && gft>0){
...
[gMPVpre_fit, gMPVcorr_fit, gGAIN_fit].each{ g->
  g.setMarkerStyle(2)
  g.setMarkerSize(4)   // int, avoids BigDecimal overload issues
}
