...
// re-plotted SUB histograms after gain calibration, and
// pT(e') vs ADC (SUB) before/after gain in a separate GUI.
//
//  - First pass:
...
//
//  Usage example:
//    run-groovy all_pd_7_bkg_fit_gain_subGain.groovy process \
...
@Field double PT_SLICE_MIN = 0.21d
@Field double PT_SLICE_MAX = 0.31d
...
@Field double ADC_MAX_2D    = 2200.0d
...
@Field Map<WireKey,H1F>   fitMap     = new LinkedHashMap<WireKey,H1F>()  // visible fit curves
...
FitResult fitDeuteronPeak(H1F h, double xmin, double xmax){
  ...
  try{
    out.chi2 = f.getChiSquare(h)
    out.ndf  = h.getAxis().getNBins() - f.getNPars()
...
if(opt.getCommand()!="process"){
  System.err.println("Usage: run-groovy all_pd_7_bkg_fit_gain_subGain.groovy process [opts] files.hipo ...")
  System.exit(1)
}
...
for(Map.Entry<WireKey,PairH> e : histMap.entrySet()){
  ...
  alphaMap.put(wk, aWire)   // store per-wire α
...
// per-wire fits + visible fit curves
for(Map.Entry<WireKey,PairH> e : histMap.entrySet()){
  WireKey wk = e.getKey()
  PairH  ph  = e.getValue()
...
// --------------------------- 3rd Pass: pT–ADC (SUB) -----------------
// build weighted SUB 2D: +1 for SIG, −α_wire for BKG; before and after gain
if(!gainMap.isEmpty() && !alphaMap.isEmpty()){
  ...
  for(String fn : files){
    ...
    while(R.hasEvent()){
      ...
      boolean okBan = true
      // we want the same SIG/BKG split as in pass1, so banana is included:
      long sumTRK = 0L
      if(ev.hasBank("AHDC::adc")){
        DataBank a0 = ev.getBank("AHDC::adc")
        for(int i=0;i<a0.rows();i++){
      ...
      okBan = (!BANANA_ON) || inBanana(pt, sumTRK)
